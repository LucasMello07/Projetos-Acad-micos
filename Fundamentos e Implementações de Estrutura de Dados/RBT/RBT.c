...
int margem = 200;
   ...
   void PercorrerRBT(RBTNode *no){
    if (no != NULL){
        DrawCircleLines(margem + no->rank * 80, margem + no->blackLevel * 80, raio, BLACK);
        DrawText((TextFormat("%d", no->key)),margem + no->rank * 80 - 12, margem + no->blackLevel * 80 - 8, 25, BLACK);

        if (no->right != NULL){
            DrawLine(margem + no->rank * 80 + (no->right->color == RBT_COLOR_RED ? 15 : 19),
            margem + no->blackLevel * 80 + (no->right->color == RBT_COLOR_RED ? 0 : 12),
            margem + no->right->rank * 80 + (no->right->color == RBT_COLOR_RED ? -15 : 0),
            margem + no->right->blackLevel * 80 + (no->right->color == RBT_COLOR_RED ? 0 : -15),
            no->right->color == RBT_COLOR_RED ? RED : BLACK );
        }

        if (no->left != NULL){
            DrawLine(margem + no->rank * 80 + (no->left->color == RBT_COLOR_RED ? -15 : -19),
            margem + no->blackLevel * 80 + (no->left->color == RBT_COLOR_RED ? 0 : 12),
            margem + no->left->rank * 80 + (no->left->color == RBT_COLOR_RED ? 15 : 0),
            margem + no->left->blackLevel * 80 + (no->left->color == RBT_COLOR_RED ? 0 : -15),
            no->left->color == RBT_COLOR_RED ? RED : BLACK );
        }

           PercorrerRBT(no->left);
            PercorrerRBT(no->right);
    }
}
