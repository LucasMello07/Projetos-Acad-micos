...
void selectionSort( GameWorld* gw ) {
    ...
    for ( size_t i = 0; i < size; i++ ) {
        ...
        createAndAddNewArrayCopy( gw ,i,-1,min);
        swap( array, min, i );
    }
...
void insertionSort( GameWorld* gw ) {
    ...
    for ( i = 1; i < size; i++ ) {
        ...
        createAndAddNewArrayCopy( gw,j,-1,i );
    }
...
void bubbleSort( GameWorld* gw ) {
    ...
    for ( size_t i = 0; i < size; i++ ) {
        ...
        createAndAddNewArrayCopy( gw , size-1, -1, j);
        if ( !swapped ) {
            break;
        }
    }
...
void shellSort( GameWorld* gw ) {
    ...
    while ( h > 0 ) {
        for ( i = h; i < size; i++ ) {
            ...
            createAndAddNewArrayCopy( gw , j, h, i);
        }
        h /= 3;
    }
...
int partition( GameWorld* gw, int start, int end ) {
    ...
    while ( true ) {
        ...
        swap( gw->array, i, j );
        createAndAddNewArrayCopy( gw, i, (i+j) /2, j );
    }
...
void heapSort( GameWorld* gw ) {
    ...
    while ( n > 0 ) {
        createAndAddNewArrayCopy( gw, 0, k, n );
        swap( gw->array, 0, n-- );
        createAndAddNewArrayCopy( gw, 0, k, n );
        sink( gw, 0, n );
    }
